...
// ================================  *
const {
  Attr, Cache, Logger, sysEnv, Constants, Events, nullValue
...
class coreSession extends Logger {
  ...
  async _selectSession() {
    if (!this.validSid()) {
      this.warn("AAAA:96 -- Invalid authorization data", this.input.authorization(), this.sid());
  ...
  async _assign_user(user) {
    ...
    this.output.set({
      status: user.signed_in ? "online" : "offline", //user.connection,
    });
    if (this._started) return;
    //if(!isEmpty(user)) this.memcache.set(this.sid(), stringify(user));
    this.trigger(START);
    this._started = 1;
  }
  ...
  async dmz_login(token, password) {
    if (!/^(share|dmz|public)$/.test(this.hub.get(Attr.area))) {
      this.warn(`AAA:348 WRONG AREA=`, this.hub.get(Attr.area));
      return {
        failed: 1,
        error: "WRONG_AREA",
      };
  ...
  refreshAuthorization() {
    if (this.input.sourceName != Attr.service) {
      return
    }
  ...
  _failover_sms(opt) {
    ...
    sms
      ...
      .then(() => {
        this.output.data({
          mobile: p.mobile.substr(p.mobile.length - 4),
          secret: otp.secret,
        });
      })
      .catch((e) => {
        this.warn("FAILED TO SEND OTP. Giving up", e);
        this.exception.server("MSG_NOT_SENT");
      });
  }
   ...
   *  @params {object} args -- extra data to be sent back to frontend
   */
  async send_otp(user, resent) {
    const Moment = require("moment");
    let p = user.profile;
    if (isString(p)) {
      p = this.parseJSON(p);
    }
    if (isEmpty(p.areacode)) {
      p.areacode = "";
    }
    ...
    let opt = {
      ...
      receivers: [p.areacode.concat(p.mobile)],
    };

    let Sms = require("./vendor/smsfactor");
    let sms = new Sms(opt);
    const timer = setTimeout(this._failover_sms.bind(this), 3500);
    sms
      .send()
      .then((result) => {
