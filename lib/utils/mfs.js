...
//
// :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: *
const { Constants } = require("@drumee/server-essentials");
const {
  INTERNAL_ERROR,
  PDFINFO,
  FOLDER,
...
const {
  existsSync, createReadStream, rmSync, mkdirSync, statSync
} = require("fs");
...
function cp(opt = "", src, dest) {
  ...
  console.log(`COPYING ${src} ==> ${dest} with '${opt}`);
  const r = cp(opt, src, dest);
...
function get_base(node) {
  const mfs_root = node.mfs_root || node.home_dir;
...
function copy_node(src, dest, detach = 0) {
  ...
  if (detach) {
    ...
    exec(`/bin/cp -rf ${src_path} ${dest_path}`);
  }
}
...
function copy_tree(src, dest) {
  ...
  check_safety(dest_path);
  const { cp } = require("shelljs");
  return cp("-rf", src_path, dest_path);
}
