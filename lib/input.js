...
const {
  Attr, uniqueId, Logger, sysEnv, nullValue
} = require("@drumee/server-essentials");

const HOMEPAGE_SVC = 'index.home';
const { tmp_dir, endpoint, instance, main_domain } = sysEnv();
...
const {
  ...
  END,
  DATA,
} = require("@drumee/server-essentials/lex/event");
    ...
    if (HTTP_PROTO.test(src)) {
      try {
        let p = src
          .replace(HTTP_PROTO, "")
          .match(/^([a-zA-Z\.\-\_]+)(\/)(.+$)/);
        let nid = this._body.nid || normalize(`/${p[3]}`);
        let vhost = this._body.vhost || `${p[1]}`;
        this.set({ nid, vhost });
      } catch (e) { }
    }
 ...
 */
  ...
  */
   ...
   */
  basepath(trailing) {
    if (trailing) return join(endpoint, trailing);
    return endpoint;
  }
  ...
  homepath(args) {
    ...
    let opt = { hostname: process.env.domain_name, ...args };
    let hostname = opt.hostname || opt.host;
